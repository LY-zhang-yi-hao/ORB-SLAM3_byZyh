...
- **目的**：定义了一个抽象接口，任何希望接收位姿更新的类都必须继承它并实现 `OnPoseUpdated` 方法。
...
public:
    ...
    virtual void OnPoseUpdated(const Sophus::SE3f& T_custom_world_camera) = 0;
};
...
### **逻辑**:
- **接口契约**: `IPoseObserver` 定义了一个抽象接口。任何希望接收位姿更新的类都必须继承它并实现 `OnPoseUpdated` 方法。这种设计遵循了面向对象编程中的**依赖倒置原则**。
...
if (mState == OK) {
    ...
    if (mpPoseObserver) {
        ...
        mpPoseObserver->OnPoseUpdated(T_custom_camera); // 调用观察者的回调函数
    }
}
```
...
public:
    void OnPoseUpdated(const Sophus::SE3f& T_custom_world_camera) override {
...
- **回调触发**: 在 `SLAM.TrackMonocular()` (或类似跟踪函数) 内部，当 `Tracking::Track()` 成功计算位姿后，它会使用存储的 `mT_custom_orb` 和当前帧位姿计算出相机在自定义坐标系下的位姿，并调用 `posePrinter.OnPoseUpdated()`。
...
- **回调执行**: 当 `Tracking` 调用 `OnPoseUpdated` 时，`MyPosePrinter` 提取 `T_custom_world_camera` 的平移部分并打印 (X, Y, Z) 位置。
